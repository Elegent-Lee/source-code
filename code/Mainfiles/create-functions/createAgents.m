...
% for i = 1:NSpecies
%     % Agent positions  unifrnd  
%     agents{1,i}(:,1) = unifrnd(PosMean(i,1),PosVar(i,1),[NAgents(i),1]); % Normal distribution with specified mean and variance for x position of all N agents
%     agents{1,i}(:,2) = unifrnd(PosMean(i,2),PosVar(i,2),[NAgents(i),1]); % Normal distribution with specified mean and variance for y position of all N agents
% 
%     % Agent Velocities
