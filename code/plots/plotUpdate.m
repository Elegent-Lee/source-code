...
x_lim = plotObj.LineLim(1:2);
y_lim = plotObj.LineLim(3:4);
...
if strcmp(settings.GammaTraj,'line')
     set(plotObj.Axes,'XLim',(x_lim + gamma_agent(1)),'YLim',(y_lim + gamma_agent(2)));
end
end
...
for i=1:NSpecies
    for j=1:NAgents(i)
    circles = simulationObj.circles;
    set(plotObj.hcircles(j,:),'Position',[circles{1,i}(j,1)-circles{1,2}(1) circles{1,i}(j,2)-circles{1,2}(1) 2*circles{1,2}(1) 2*circles{1,2}(1)]);
    end
end
